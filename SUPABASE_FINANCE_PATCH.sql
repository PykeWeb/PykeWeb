...
begin
  if to_regclass('public.expenses') is not null then
    execute 'create index if not exists expenses_group_created_idx on public.expenses(group_id, created_at desc)';
  end if;
  if to_regclass('public.transactions') is not null then
    execute 'create index if not exists transactions_group_created_idx on public.transactions(group_id, created_at desc)';
  end if;
  if to_regclass('public.weapon_stock_movements') is not null then
    execute 'create index if not exists weapon_stock_movements_group_created_idx on public.weapon_stock_movements(group_id, created_at desc)';
  end if;
  if to_regclass('public.equipment_stock_movements') is not null then
    execute 'create index if not exists equipment_stock_movements_group_created_idx on public.equipment_stock_movements(group_id, created_at desc)';
  end if;
  if to_regclass('public.drug_stock_movements') is not null then
    execute 'create index if not exists drug_stock_movements_group_created_idx on public.drug_stock_movements(group_id, created_at desc)';
  end if;
end $$;
...
begin
    ...
    if to_regclass('public.transaction_items') is not null then
      ...
      sql := sql || $q$
        ...
        select
          ...
          case when t.type = 'purchase' then 'purchase' else 'sale' end as movement_type,
          'objects'::text as category,
          'Transaction'::text as item_label,
          t.counterparty as member_name,
    ...
    if to_regclass('public.weapons') is not null then
      ...
      sql := sql || $q$
        ...
        select
          ...
          case when wsm.delta >= 0 then 'purchase' else 'sale' end as movement_type,
          'weapons'::text as category,
          coalesce(wsm.note, 'Mouvement arme') as item_label,
          null::text as member_name,
          abs(wsm.delta)::numeric as quantity,
          null::numeric as amount,
          null::text as expense_status,
    ...
    if to_regclass('public.equipment') is not null then
      ...
      sql := sql || $q$
        ...
        select
          ...
          case when esm.delta >= 0 then 'purchase' else 'sale' end as movement_type,
          'equipment'::text as category,
          coalesce(esm.note, 'Mouvement équipement') as item_label,
          null::text as member_name,
          abs(esm.delta)::numeric as quantity,
          null::numeric as amount,
          null::text as expense_status,
  ...
  if to_regclass('public.drug_stock_movements') is not null then
    if to_regclass('public.drug_items') is not null then
      sql := sql || $q$
        union all
        select
          dsm.id,
      ...
      sql := sql || $q$
        ...
        select
          ...
          case when dsm.delta >= 0 then 'purchase' else 'sale' end as movement_type,
          'drugs'::text as category,
          coalesce(dsm.note, 'Mouvement drogue') as item_label,
          null::text as member_name,
          abs(dsm.delta)::numeric as quantity,
          null::numeric as amount,
          null::text as expense_status,
