...
function resolveRequiredPrefix(pathname: string) {
  ...
  if (path === '/tablette' || path.startsWith('/tablette/')) return '/tablette'
...
export function hasRequiredPrefixAccess(allowedPrefixes: string[], requiredPrefix: string) {
  ...
  if (requiredPrefix === '/tablette') return prefixes.includes('/tablette') || prefixes.includes('/operations') || prefixes.includes('/activites')
