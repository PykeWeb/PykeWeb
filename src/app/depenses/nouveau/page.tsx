...
type PickItem =
  | { type: 'object'; id: string; name: string; price: number }
  | { type: 'weapon'; id: string; name: string; price: number }
  | { type: 'equipment'; id: string; name: string; price: number }
  | { type: 'drug'; id: string; name: string; price: number }

export default function NouvelleDepensePage() {
  const router = useRouter()

  const [memberName, setMemberName] = useState('')
  const [itemType, setItemType] = useState<ExpenseItemType>('object')
    ...
    async function load() {
      ...
      try {
        ...
        if (itemType === 'object') {
          ...
          setItems(
            data.map((o: DbObject) => ({ type: 'object', id: o.id, name: o.name, price: o.price })) as any
          )
        } else if (itemType === 'weapon') {
          const data = await listWeapons()
          setItems(
            data.map((w: DbWeapon) => ({ type: 'weapon', id: w.id, name: w.name || w.weapon_id || 'Arme', price: 0 })) as any
          )
          ...
          setItems(
            data.map((d: DbDrugItem) => ({ type: 'drug', id: d.id, name: d.name, price: d.price })) as any
          )
        }
      } catch (e: any) {
