...
export function QuantityStepper({ value, onChange, min = 1, max, size = 'default', fitContent = false }: Props) {
  ...
  const inputClassName = size === 'sm' ? 'h-7 min-w-[2.75rem] px-1 text-center text-sm font-semibold text-white' : 'w-24'
  const fitWidthStyle = fitContent ? { width: `${Math.max(size === 'sm' ? 6 : 3, draft.length + (size === 'sm' ? 3 : 1))}ch` } : undefined
